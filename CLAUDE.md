...
A static single-page marketing site for Embrayse — aged care food services software. The active implementation lives in `site/`. No build tools, no framework. Vanilla HTML/CSS/JS.

Legacy versions (v1, v2, etc.) remain in the root directory but are **not the active design**. The current design direction is defined by the Figma file.
...
2. **Use pre-exported assets** — All assets are already exported in `site/assets/`. Most are @2x, some @4x. Size `<img>` elements to match their Figma frame dimensions (not the retina pixel dimensions).
...
21. **hr** separator
22. Book Demo + Mission (scheduling widget + company mission + Brisbane skyline)
23. Footer
...
A ranked list of target SEO keywords lives in `site/SEOKeywords`. Reference this when writing or refining copy — prioritise higher-ranked terms where they fit naturally. Not every keyword needs to appear; use judgement based on the section context and search intent.
