...
The Embrayse marketing site, aged care food services software. Hosted via Netlify from Azure DevOps.

The main site (home, legal, release-notes, app-support) is vanilla HTML/CSS/JS with no build step. The `/insights/` content section is generated by Eleventy (11ty) from markdown in `content/`. Everything else is static.

## Running locally

```bash
# Build /insights/ (once after editing content):
npm install            # first time only
npm run build          # regenerates the insights/ folder

# Serve the whole site with live reload (preferred):
npx live-server .

# Eleventy dev server (watches content/, hot reloads /insights/):
npm run dev
```

Use `live-server` when working on the main site. Use `npm run dev` when iterating on insights posts or templates.
...
`/insights/` is generated by Eleventy from markdown sources in `content/`.

```
content/                              # sources (committed)
  _includes/                          # base, post, card-grid templates
  _data/site.json                     # site-wide vars
  tags/tag.njk                        # tag archive template
  feed.njk                            # RSS feed
  index.njk                           # /insights/ listing page
  {slug}/index.md                     # one post per folder, frontmatter + body
  {slug}/hero.jpg                     # hero image per post

insights/                             # 11ty output (gitignored)
eleventy.config.js                    # build config
package.json, netlify.toml            # build + deploy config
insights.css                          # card grid, hero, tag pills, closing CTA
```

**Adding a new post:** create `content/{slug}/index.md` with frontmatter (title, date, excerpt, tags, hero, hero_alt) plus a hero image in the same folder. Run `npm run build` and reload `/insights/`. The slug is the folder name, so it becomes the URL: `/insights/{slug}`.

Netlify runs `npm run build` on every deploy and publishes the repo root, so the generated `insights/` folder is served alongside the static pages.
