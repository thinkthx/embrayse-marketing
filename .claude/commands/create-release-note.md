...
Images referenced in `[screenshot: ...]` tags should be provided by the engineer as file paths (relative or absolute) or already placed in `release-notes/assets/{version-slug}/`.
...
If `release-notes/{version-slug}.html` already exists, stop and ask the engineer whether to overwrite. Never overwrite silently.

### 3. Set up screenshot directory

Create `release-notes/assets/{version-slug}/` if it doesn't exist.

For each screenshot referenced in the input:
- If the engineer provided absolute/relative paths, copy the files into `release-notes/assets/{version-slug}/` using `cp`.
...
Create `release-notes/{version-slug}.html` using this exact template. Replace placeholders (in `{{...}}`) with the polished content.
...
Read `release-notes/index.html` and insert the new entry at the top of the correct year group's `<ul class="rn-list">`.
...
4. Tell the engineer to run `npx live-server .` and visit `/release-notes/{{version-slug}}` to preview

Report a summary like:
```
Release {{version}} created:
• Page: release-notes/{{version-slug}}.html
• Screenshots: {{count}} copied to release-notes/assets/{{version-slug}}/
